    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if (
                ...
                and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 180
            ):
...
@bot.message_handler(commands=["server_free"])
def handle_server_free_test(message):
    if check_process_with_bgmi():
        response = "Please Wait a while the current process is finished."
    else:
...
def show_help(message):
 ...
 /server_test : Check if server is free.
...
def welcome_plan(message):
 ...
 /stop : stop all the ongoing processes.
"""
    bot.reply_to(message, response)
